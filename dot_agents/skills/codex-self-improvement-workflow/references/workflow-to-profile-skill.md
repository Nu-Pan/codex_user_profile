...
- ユーザーが提示した再利用可能な workflow を、Codex が繰り返し扱える `profile`、`束ね skill`、`役割別 skill` に分解するときの基準を定義する。
...
- `束ね skill`
  - workflow 全体の導線、推奨フェーズ順、読むべき reference、役割別 skill への入口を与える。
- `役割別 skill`
  - 1 つのフェーズ、観点、責務に閉じた詳細手順、判断基準、テンプレ断片を与える。

## Default story

- 既定では、新しい典型 workflow は `profile` と 1 つの `束ね skill` で表現する。
- workflow に複数の責務やレビュー観点がある場合は、`束ね skill` の下に複数の `役割別 skill` を置く。
- `profile` は session 契約だけを持ち、workflow の本文や役割分担を抱え込まない。
- `束ね skill` は「最初に何を読み、どの順で進め、必要ならどの skill へ降りるか」を示す。
- `役割別 skill` は、その責務に必要な入力条件、期待出力、判断基準だけに閉じる。
- フェーズ順は既定では推奨順序であり、hard gate にはしない。往復や省略があり得る場合は `束ね skill` 側で条件を説明する。
- 同じ workflow を profile だけで表現しようとして `developer_instructions` が長くなるなら、詳細は `束ね skill` と `役割別 skill` に逃がす。
...
- 途中で読むべき reference や、役割別 skill の選び方を再利用したい。
...
- `developer_instructions` から読ませる入口が 1 つで十分で、役割別 skill を経由させる理由が弱い。

## When to add profile + multiple skills

- session 契約の追加と、workflow 全体の導線と、再利用可能な役割別手順のすべてが必要である。
- 既定では、複数責務を持つ workflow は `profile` + `束ね skill` + 必要な `役割別 skill` の組で表現する。
- 例外として、単なる durable 設定差分だけなら profile だけでよい。
- 例外として、既存 profile の行動契約で十分なら、`束ね skill` と `役割別 skill` だけでよい。
...
- 既定では、must-read には `AGENTS.md` と `束ね skill` だけを明示する。
- `役割別 skill` を直接 must-read に並べるのは例外であり、`束ね skill` を経由すると入口が壊れる場合だけに限る。
- 詳細 workflow や長いテンプレ本文は書かない。
- 詳細は `束ね skill` と、そこから辿る関連 skill / `references/` を参照する一文に留める。

### `~/.agents/skills/<bundle-name>/SKILL.md`

- skill 名は `lower-hyphen-case` を既定にする。
- trigger 条件、目的、推奨フェーズ順、役割別 skill の導線、reference map を置く。
- session 契約の正本にはならない。
- 各フェーズの細かい手順は抱え込まず、必要な `役割別 skill` や reference への入口に留める。
...
- end-to-end の段取りやフェーズ間の受け渡しは `束ね skill` 側の reference に置く。
...
- workflow から end-to-end の導線、推奨フェーズ順、役割分担を抜き出し、`束ね skill` に入れる。
- workflow から再利用可能な局所手順と判断基準を抜き出し、`役割別 skill` と `references/` に入れる。
- durable 設定だけを抜き出し、`config.toml` に入れる。
- repo-wide router が本当に必要かを最後に判断し、必要なときだけ `AGENTS.md` を触る。
- 同じ rule を `developer_instructions`、`束ね skill`、`役割別 skill` の複数箇所へ重複させない。
...
- 詳細な workflow、推奨フェーズ順、役割別 skill への導線は user skill `<bundle-skill-name>` と、そこから辿る関連 skill / `references/` を参照する。
...
- この skill は <workflow> 全体の導線、推奨フェーズ順、役割別 skill への入口を定義する。
...
- `束ね skill` は、workflow 全文の置き場所ではなく、入口と導線の置き場所として使う。
- `束ね skill` が長くなり始めたら、詳細手順は `役割別 skill` 側の skill / reference へ移す。
...
- workflow 全体の導線は対応する `束ね skill` を正本とする。
...
- `役割別 skill` は、他フェーズの段取りや session 契約を再掲しない。
...
- `profile`
  - 目的は「高品質な修正作業を行う」。
  - must-read は `AGENTS.md` と修正 workflow の `束ね skill` に留める。
- `束ね skill`
...
- `codex exec '$<bundle-skill-name> Summarize this workflow, the recommended phases, and which component skills you would read.'` で `束ね skill` の導線を確認する。
- `codex exec '$<component-skill-name> Summarize your phase, inputs, outputs, and review criteria.'` で `役割別 skill` の局所責務を確認する。
